...
object LivenessAnalysis {
  ...
  /** join(s) = \sqbigcup_{t \in succ(s)} t
    *
    * @param succs,
    *   abstract states coming from the successors
    * @return
    */
  // Task 2.1
  def join(succs: List[AbstractState]): AbstractState =
    succs.foldLeft(Set.empty[String])(_ union _)
  ...
  def genMonotoneFunction(p: List[LabeledInstr]): MonotoneFunction = {
    ...
    def joinSuccStates(label: Label, env: AbstractEnv): AbstractState = {
      ...
      val succs_states: List[AbstractState] = succs.flatMap(succ =>
        env.get(succ) match {
          case None        => List(Set())
    ...
    (absEnv: AbstractEnv) => {
      ...
      foldM((acc: AbstractEnv, li: LabeledInstr) =>
        li match {
          ...
          case (label, IMove(Temp(AVar(t)), src)) => {
            ...
            Right(
              acc + (label -> (joined_succs_states - t union vars(src).toSet))
            )
          }
          /** case l: t <- src1 op src2: s_l = join(s_l) - {t} \cup vars(src1)
            * \cup vars(src2)
            */
          // Task 2.2
          ...
          case (label, IPlus(Temp(AVar(t)), src1, src2)) => {
            val joined_succs_states = joinSuccStates(label, acc)
            Right(
              acc + (label -> (joined_succs_states - t union vars(
                src1
              ).toSet union vars(src2).toSet))
            )
          }
          ...
          /** case l: r <- src1 op src2: s_l = join(s_l) \cup vars(src1) \cup
            * vars(src2)
            */
          case (label, IPlus(Regstr(n), src1, src2)) => {
            val joined_succs_states = joinSuccStates(label, acc)
            Right(
              acc + (label -> (joined_succs_states union vars(
                src1
              ).toSet union vars(src2).toSet))
            )
          }
          ...
          case (label, _) => {
            val joined_succs_states = joinSuccStates(label, acc)
            Right(acc + (label -> joined_succs_states)): Either[
              String,
              AbstractEnv
            ]
          }
  ...
  /** Top level function for liveness analysis Peform liveness analysis over a
    * PA program `p` return an abstract environment mapping label to abstract
    * states Each abstract state is set of variable names
    *
    * @param p
    * @param i
    * @return
  ...
  def analyze(
      p: List[LabeledInstr]
  )(using i: CompleteLattice[AbstractEnv]): Either[String, AbstractEnv] = {
    val f = genMonotoneFunction(p)
    val labels = p.map(li => li match { case (l, _) => l })
    val init_abstract_state: AbstractState = Set()
    val init_abstract_env: AbstractEnv =
      labels.map(l => (l, init_abstract_state)).toMap
