...
object TypeInf {
  ...
  /** extract type variable names from a set of type constraints
    ...
    *   a set of names
    */
  def getTVars(tcs: TypeConstrs): Set[String] = tcs.toList
    .flatMap(p =>
      p match {
  ...
  /** Apply a type substutiton to an extended type
    ...
    * @param type
    * @return
    *   either an error or a grounded monotype
    */
  given exTypeSubstitutable: Substitutable[ExType] = new Substitutable[ExType] {
    def applySubst(tysubst: TypeSubst)(ty: ExType): ExType = tysubst match {
      // Lab 2 Task 2.1
      case TypeSubst.Empty => ty
      case TypeSubst.RevComp((n, ty1), psi1) =>
        val typ = ty match
          case TypeVar(m) =>
            if (m == n) {
              ty1
  ...
  /** Apply a type substution to a pair of things
    */
  given pairSubstitutable[A, B](using ia: Substitutable[A])(using
      ib: Substitutable[B]
  ): Substitutable[(A, B)] = new Substitutable[(A, B)] {
  ...
  given listSubstitutable[A](using
      i: Substitutable[A]
  ): Substitutable[List[A]] = new Substitutable[List[A]] {
    def applySubst(typeSubst: TypeSubst)(la: List[A]): List[A] =
      la.map(a => i.applySubst(typeSubst)(a))
  }
  ...
  given infStmt: Infer[Stmt] = new Infer[Stmt] {
    def infer(s: Stmt): TypeConstrs = s match {
      ...
      // If, While
      case If(cond, th, el) => {
        val k1 = inferExp(cond) match {
          case (exTy, k) => k + ((MonoType(BoolTy), exTy))
        }
        val k2 = summon[Infer[List[Stmt]]].infer(th)
        val k3 = summon[Infer[List[Stmt]]].infer(el)
        k1 union k2 union k3
      }
      case While(cond, b) => {
        val k1 = inferExp(cond) match {
          case (exTy, k) => k + ((MonoType(BoolTy), exTy))
        }
        val k2 = summon[Infer[List[Stmt]]].infer(b)
        k1 union k2
      }
      // Lab 2 Task 2.3 end
    }
  ...
  def inferExp(e: Exp): (ExType, TypeConstrs) = e match {
    ...
    // Lab 2 Task 2.3
    case Plus(e1, e2) => {
      val k1 = inferExp(e1) match
        case (exty, k) => k + ((MonoType(IntTy), exty))
      val k2 = inferExp(e2) match
        case (exty, k) => k + ((MonoType(IntTy), exty))
      (MonoType(IntTy), k1 union k2)
    }
    case Minus(e1, e2) => {
      val k1 = inferExp(e1) match
        case (exty, k) => k + ((MonoType(IntTy), exty))
  ...
  /** unifying two ExTypes
    */
  given extypesUnifiable: Unifiable[(ExType, ExType)] =
    new Unifiable[(ExType, ExType)] {
      def mgu(p: (ExType, ExType)): Either[String, TypeSubst] = p match {
        ...
        // case (TypeVar(alpha), TypeVar(beta)) if alpha == beta => Right(Empty)
        case (TypeVar(alpha), ty) => Right(single(alpha, ty))
        case (ty, TypeVar(alpha)) => Right(single(alpha, ty))
  ...
  /** unifying a set of type constraints (i.e. a set of (ExType, ExType))
    */
  given typeConstrsUnifiable: Unifiable[TypeConstrs] =
    new Unifiable[TypeConstrs] {
      def mgu(tyconstrs: TypeConstrs): Either[String, TypeSubst] = {
        listUnifiable.mgu(tyconstrs.toList)
      }
    }

  given listUnifiable[A](using u: Unifiable[A])(using
      s: Substitutable[List[A]]
  ): Unifiable[List[A]] = new Unifiable[List[A]] {
  ...
  /** grounding a type variable's name given a type substitution
    ...
    * @return
    */
  def ground(varname: String, subst: TypeSubst)(using
      i: Substitutable[ExType]
  ): Either[String, Type] = i.applySubst(subst)(TypeVar(varname)) match {
    case MonoType(t) => Right(t)
    case _ =>
      Left(
        s"error: type inference failed. ${varname}'s type cannot be grounded ${subst}."
      )
  }
  ...
  /** top level type inference function
    *
    * @param s
    *   \- list of SIMP Statements
    * @param i
    *   \- inference statement type class instance
    * @param u
    *   \- unifiable type class instance
    * @return
    *   either an error or a type environment
    */
  def typeInf(s: List[Stmt])(using i: Infer[List[Stmt]])(using
      st: Substitutable[ExType]
  )(using u: Unifiable[TypeConstrs]): Either[String, TypeEnv] = {
