...
object MMUpDown {
  ...
  def genExp(e: Exp)(using
      me: StateCogenMonad[StateInfo]
  ): CogenState[(Opr, List[(Label, Instr)])] = e match {
    // GE(c) |- (c, [])      (Const)
    ...
    case Minus(e1, e2) =>
      for {
        ...
        val instr = (l, IMinus(x, up1, up2))
        (x, down1 ++ down2 :+ instr)
      }

    // The rest? Exception?
    case _ => me.pure((IntLit(0), Nil)) // fixme
    // Lab 1 Task 2.1 end
  }
  ...
  def cogen(s: Stmt): CogenState[List[(Label, Instr)]] = s match {
    ...
    case While(cond, b) =>
      for {
        ...
        (upCond, downCond) <- genExp(cond)
        LWhileCondJ <- newLabel
        instrs2 <- cogen(b)
        LEndBody <- newLabel
        LEndWhile <- chkNextLabel
      } yield {
        val instrs1 = List((LWhileCondJ, IIfNot(upCond, LEndWhile)))
        val instrs2p = instrs2 ++ List((LEndBody, IGoto(LBWhile)))
        downCond ++ instrs1 ++ instrs2p
      }

    // exception? I think will never reach
