...
object Parser {
  ...
  /** S ::= X = E ; | return X ; | nop | if E { \overline{S} } else {
    * \overline{S} } | while E { \overline{S} } E ::= E Op E | X | C | (E)
    * \overline{S} ::= S | S \overline{S} Op ::= + | - | * C ::= 1 | 2 | ... |
    * true | false X ::= a | b | c | d
    */
  ...
  /** Parsing a sequence of statements, we skip the preceeding and the
    * proceeding white spaces for each statement. for individual statement
    * parser, we only need to skip the whitespace in between.
    *
    * @return
    */
  ...
  /** Parsing a statement
    ...
    * @return
    */
  def p_stmt: Parser[PEnv, Stmt] =
    choice(p_assign)(choice(p_ret)(choice(p_nop)(choice(p_ifelse)(p_while))))
  ...
  /** Lab 1 Task 1.1 parsing / skipping whitespaces
    *
    * @return
    */
  def p_space: Parser[PEnv, LToken] = sat(ltoken => // TODO 1.1.1
    ltoken match {
  ...
  def p_spaces: Parser[PEnv, List[LToken]] = { // TODO 1.1.2
    many(p_space)
  }

  /** Lab 1 Task 1.1 end */

  /** Lab 1 Task 1.2 Parsing an expression Note that E ::= E Op E | X | C | (E)
  ...
  /** Parsing operator symbols
    ...
    * @return
    */
  def p_plus: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_minus: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_mult: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_lthan: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_dequal: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
      case DEqSign(_) => true
      case _          => false
    }
  ...
  def p_equal: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
      case EqSign(_) => true
      case _         => false
    }
  ...
  def p_var: Parser[PEnv, Var] = for {
    tok <- sat((ltoken: LToken) =>
      ltoken match {
        case IdTok(src, v) => true
        case _             => false
      }
    )
    name <- someOrFail(tok)(t =>
      t match {
        case IdTok(src, v) => Some(v)
        case _             => None
      }
    )(
      "error: expecting an identifier, but None is returned."
  ...
  def p_int: Parser[PEnv, Const] = for {
    ...
    i <- someOrFail(tok)(t =>
      ...
      "error: expecting an integer, but None is returned."
    ) // this error should never occur.
  } yield IntConst(i)
  ...
  /** Parsing keywords
    ...
    * @return
    */
  def p_returnKW: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
      case RetKW(src) => true
      case _          => false
    }
  ...
  def p_ifKW: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
      case IfKW(src) => true
      case _         => false
    }
  ...
  def p_elseKW: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_whileKW: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  /** Parsing symbols
    */
  def p_lbrace: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_rbrace: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_lparen: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_rparen: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
  ...
  def p_semicolon: Parser[PEnv, LToken] = sat(ltoken =>
    ltoken match {
