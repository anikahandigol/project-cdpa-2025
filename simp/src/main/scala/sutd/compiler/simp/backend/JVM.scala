...
/** implementation of the JVM code generator
  ...
  *
  * Due to the api limitation of the org.objectweb.asm.ClassVisitor being a JAVA
  * object we need to implement the code generator as a function with effect,
  * i.e. return MVStateMonad[Unit] instead of return a list of JVM instructions.
  *
  * In this implementation, each call to convert function return
  * MVStateMonad[Unit] must be strictly ordered in order to ensure the correct
  * sequence of JVM instructions being generated.
  */
...
object JVM {
  ...
  def init_mv_stateInfo(): MVStateInfo = {
    ...
    ) // cOMPUTE_MAXS automatically compute stack size, COMPUTE_FRAMES automatically compute the stackmap
  ...
  def get: StateT[MVStateInfo, EitherM[String], MVStateInfo] = StateT { st =>
    Right(st, st)
  }
  def put(st: MVStateInfo): StateT[MVStateInfo, EitherM[String], Unit] =
    StateT { _ => Right(st, ()) }
  ...
  /** retrieve the method visitor from the state info object
    ...
    * @return
    */
  def getMV: StateT[MVStateInfo, EitherM[String], MethodVisitor] = StateT {
    st =>
      st match {
        case MVStateInfo(true, mv, cw) =>
          Left("getMV failed. Method Visitor has been ended.")
        case MVStateInfo(ended, mv, cw) => Right((st, mv))
      }
  }

  /** the L environment. It differs from the note as follows.
    *   1. It is not a set of PA labels 2. It is a mapping from PA labels to JVM
    *      Labels
    */
  ...
  /** convert a PA label `lbl`
    ...
    * @return
    */
  def convertLabel(lenv: L, lbl: Label)(using
      m: StateMVMonadError[MVStateInfo]
  ): MVState[Unit] = lenv.get(lbl) match {
    /** l \not \in L (jLabel1) --------------- L |- l => []
      */
    case None => m.pure(())
    /** (l, jvm_l) \in L (jLabel2) --------------------- L |- l => [ilabel
      * jvm_l]
      */
    case Some(jvm_lbl) =>
      for {
  ...
  /** covert a PA operand
    ...
    * @return
    */
  def convertOpr(menv: M, opr: Opr)(using
      m: StateMVMonadError[MVStateInfo]
  ): MVState[Unit] = opr match {
    /** (jConst) M |- c => [sipush c]
      */
    case IntLit(c) =>
      for {
    ...
    case Temp(AVar(t)) =>
      menv.get(t) match {
        case None =>
          m.raiseError(
            s"convertOpr failed: temp variable ${t} is allocated to any local variable slot."
          )
        case Some(jvar) =>
          for {
            mv <- getMV
    ...
    case Regstr(name) =>
      m.raiseError(
        s"convertOpr failed: register ${name} should not appear as an operands in Pseudo Assembly."
      )
  }
  ...
  /** converting a sequence of PA labeled instructions
    ...
    * @return
    */
  def convertInstr(menv: M, lenv: L, lis: List[LabeledInstr])(using
      m: StateMVMonadError[MVStateInfo]
  ): MVState[Unit] = lis match {
    /** L |- l1 => jis0 M |- s => jis1 (jReturn)
      * -------------------------------------------------------------------- M,
      * L |- l1 : rret <- s; l2 : ret => jis0 + jis1 + [print s; return]
      */
    case ((l1, IMove(Regstr("_r_ret"), src)) :: (l2, IRet) :: Nil) =>
      for {
        ...
        _ <- src match {
          case Temp(AVar(t)) =>
            menv.get(t) match {
              ...
              case None =>
                m.raiseError(
                  s"convertInstr failed: temp variable ${t} is not mapped to any jvm local variable."
                )
            }
          case Regstr(name) =>
            m.raiseError(
          ...
          case IntLit(v) =>
            m.raiseError(
              s"convertOpr failed: integer constant ${v} should not appear as an operands of return in Pseudo Assembly."
            )
        }
        _ <- end // must end
      } yield ()
    /** L |- l => jis0 M |- s => jis1 M,L |- lis => jis2 (jMove)
      * ----------------------------------------------------------------- M, L
      * \|- l : t <- s ; lis => jis0 + jis1 + [istore M(t)] + jis2
      */
    case ((l, IMove(Temp(AVar(t)), src)) :: lis) =>
      menv.get(t) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: temp variable ${t} is not mapped to any jvm local variable."
    ...
    /** L |- l => jis0 M |- s1 => jis1 M |- s2 => jis2 M,L |- lis => jis3 (jAdd)
      * -----------------------------------------------------------------------
      * M, L |- l : t <- s1 + s2; lis => jis0 + jis1 + jis2 + [iadd, istore
      * M(t)] + jis3
      */
    case ((l, IPlus(Temp(AVar(t)), src1, src2)) :: lis) =>
      menv.get(t) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: temp variable ${t} is not mapped to any jvm local variable."
    ...
    /** L |- l => jis0 M |- s1 => jis1 M |- s2 => jis2 M,L |- lis => jis3 (jSub)
      * -----------------------------------------------------------------------
      * M, L |- l : t <- s1 - s2; lis => jis0 + jis1 + jis2 + [isub, istore
      * M(t)] + jis3
      */
    case ((l, IMinus(Temp(AVar(t)), src1, src2)) :: lis) =>
      menv.get(t) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: temp variable ${t} is not mapped to any jvm local variable."
    ...
    /** L |- l => jis0 M |- s1 => jis1 M |- s2 => jis2 M,L |- lis => jis3
      * (jMult)
      * -----------------------------------------------------------------------
      * M, L |- l : t <- s1 * s2; lis => jis0 + jis1 + jis2 + [imul, istore
      * M(t)] + jis3
      */
    case ((l1, IMult(Temp(AVar(t)), src1, src2)) :: lis) =>
      menv.get(t) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: temp variable ${t} is not mapped to any jvm local variable."
    ...
    /** L |- l1 => jis0 M |- s1 => jis1 M |- s2 => jis2 M,L |- lis => jis3 (jEq)
      * ----------------------------------------------------------------------------------------------------
      * M, L |- l1:t <- s1 == s2; l2: ifn t got l3; lis => jis0 + jis1 + jis2 +
      * [if_icmpne L(l3)] + jis3
      */
    case ((l1, IDEqual(Temp(AVar(t)), src1, src2)) :: (
          l2,
          IIfNot(Temp(AVar(t2)), l3)
        ) :: lis) if t == t2 =>
      lenv.get(l3) match {
    ...
    /** L |- l1 => jis0 M |- s1 => jis1 M |- s2 => jis2 M,L |- lis => jis3
      * (jLThan)
      * ----------------------------------------------------------------------------------------------------
      * M, L |- l1:t <- s1 < s2; l2: ifn t goto l3; lis => jis0 + jis1 + jis2 +
      * [if_icmpge L(l3)] + jis3
      */
    case ((l1, ILThan(Temp(AVar(t)), src1, src2))) ::
        ((l2, IIfNot(Temp(AVar(t2)), l3))) ::
        lis if t == t2 =>
      lenv.get(l3) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: label $l3 is not mapped to any jvm label."
          )
    ...
    /** L |- i1 => jis0 M, L |- lis => jis1 (jGoto)
      * ---------------------------------------------------------- M, L |- l1:
      * goto l2; lis => jis0 + [igoto L(l2)] + jis1
      */
    case ((l1, IGoto(l2)) :: lis) =>
      lenv.get(l2) match {
        case None =>
          m.raiseError(
            s"convertInstr failed: label ${l2} is not mapped to jvm label."
          )
        case Some(jl2) =>
          for {
            _ <- convertLabel(lenv, l1)
  ...
  def generateLEnv(lis: List[LabeledInstr]): L = {
    def getGotoLabels(lis: List[LabeledInstr]): List[Label] = lis
      .flatMap(li =>
        li match {
          case (_, instr) =>
            instr match {
  ...
  /** Top level functoin to generate the target code
    ...
    * @param lis
    * @return
    *   either an unit () or an error message.
    */
